...
const Checkout = (props) => {
  ...
  const confirmHandler = (event) => {
    ...
    const enteredNameIsValid = !isEmpty(enteredName); //valid if not empty
    const enteredStreetIsValid = !isEmpty(enteredStreet);
    const enteredPostalCodeIsValid = isFiveChars(enteredPostalCode); //valid if has 5 chars
    const enteredCityIsValid = !isEmpty(enteredCity);

    // Setting each inputs validity
